...
class ModbusServer:
    def __init__(self, host='0.0.0.0', port=502):
        ...
        self.db_path = os.path.join(nordpool_dir, "data", "nordpool_prices.db")
        self.devices_path = os.path.join(wifi_dir, "config", "devices.json")
    ...
    def load_price_data(self):
        ...
        try:
            ...
            print(f"Error loading price data from database: {e}")
    
    def load_device_data(self):
        """Load device data from the devices.json file and update registers"""
        try:
            ...
            for device in devices:
                ...
                if 1 <= device_id <= 30:
                    self.registers[3000 + device_id - 1] = 1 if online else 0
            
            print(f"Loaded device data: {len(devices)} devices")
        except Exception as e:
